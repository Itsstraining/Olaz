...
export class RejectAddComponent implements OnInit {
  ...
  handleError(e: any) {
    console.log(e);
    e.target.src =
      'https://cdyduochopluc.edu.vn/wp-content/uploads/2019/07/anh-dai-dien-FB-200-1.jpg';
  }
  ...
  async getAllRequests(myID: string) {
    ...
    user['requests'].map(async (requestId: string, index: number) => {
      ...
      user['requests'][index] = await (
        await this.UserService.getUserByID(requestId)
      ).data();
    });
  ...
  addFriend(frID: string) {
    if (this._user) {
      this.UserService.sendRequest(this._user.id, frID).subscribe(
        (response) => {
